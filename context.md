...
> 最後更新：2026-04-17（分類名稱白話化 + 三層定價重構 + 首頁文案/header 修改。總題庫 7,393 題 across 191 exams）
...
### 2.1 定價結構（2026-04-17 確定版）

| 方案 | 30 天 | 90 天 | 定位 |
|---|---|---|---|
| 免費版 | NT$0 | — | 體驗入門，每日 10 題，需註冊 |
| 標準方案 | NT$99 / 30 天 | NT$249 / 90 天（省 16%） | 價格敏感用戶，技術士 + 金融證照 |
| 完整方案 | NT$249 / 30 天 | NT$599 / 90 天（省 20%） | 高付費意願用戶，全功能 |

> **計費週期**：採天數制，從付費當下起算 30 天 / 90 天到期，不綁月份（避免月底購買吃虧的感受）。內部 `plan_code` 仍沿用 `standard_monthly` / `standard_quarterly` 等舊命名以維持既有訂單相容性。

### 2.2 功能分層

| 功能 | 免費版 | 標準方案 | 完整方案 |
|---|---|---|---|
| 題庫練習 | 所有考試每日 10 題 | 技術士 + 金融證照無限練習 | 所有考試無限練習 |
| 主題練習與隨機測驗 | 基本 | ✅ | ✅ 無限制 |
| 模擬考 | ❌ | ✅ 適用範圍內完整模擬考 | ✅ 所有考試 |
| 答題解析 | ✅ | ✅ | ✅ + AI 補充說明 |
| AI 弱點分析 | ❌ | ✅ | ✅ + 個人化學習計畫 |
...
僅限以下兩類考試（價格敏感、通常只需一科）：
- **技術士證照**（tech-cert）：61 科，用戶為高職生/基層勞工
- **金融證照**（finance）：28 科，用戶為銀行/保險新進員工

不開放標準方案的類別（付費意願高，不需降價）：專技人員考試、公職考試、語言檢定、升學考試、IT 國際認證

### 2.4 定價策略決策紀錄

- **競品參考**：阿摩 VIP 約 NT$130-140/月，有 iOS+Android app，內容為 UGC 模式品質參差
- **定價高於阿摩的理由**：AI 功能、結構化題庫、模擬考、弱點分析，品質定位更高
- **設定較高定價的考量**：日常價格留出折扣空間，後續可做「首購 NT$69 / 30 天」「限時七折」等促銷活動
- **標準方案存在理由**：降低技職/金融用戶門檻，避免托福/高普考用戶降級（限定適用科目）
- **考前衝刺包不放入標準方案的理由**：最佳升級觸發點——考前 2-3 週焦慮感上來時，用戶最容易轉換
- **升級路徑設計**：標準用戶升級完整方案時，按比例折抵剩餘天數

### 2.5 收入預估（Year 1）

| 情境 | 月活付費用戶 | 平均月費 | 月收入 | 年收入 |
|---|---|---|---|---|
| 保守 | 30-50 人 | NT$130 | NT$5,000-6,500 | NT$6-8 萬 |
| 中等 | 100-200 人 | NT$150 | NT$15,000-30,000 | NT$20-35 萬 |
| 樂觀 | 500+ 人 | NT$180 | NT$90,000+ | NT$100 萬+ |

### 2.6 新用戶 7 天 Pro 試用（v13，2026-04-20 上線）

- **範圍**：僅新註冊用戶自動開通，既有 free 用戶不補發。
- **額度**：7 天 + 200 題（任何模式皆計入，到期或達 200 題先到者結束）。
- **結束後**：降回免費版並彈付費 Modal；不進 subscriptions 表、無金流紀錄。
- **實作策略**：`current_tier` 仍為 `'free'`（不破壞既有 CHECK constraint），試用採前端覆蓋式 overlay——`billing.getTier()` 若 `trial_expires_at > now() && used < 200`，即回 `{tier:'pro', isTrial:true}`。付費後 webhook 把 `current_tier` 改為 `standard/pro`，試用自然失效。
- **防薅羊毛**：200 題額度本身即為上限；未另加手機驗證，轉換成本 > SMS 費用。
- **相關檔**：`supabase_schema_v13_trial.sql`、`profiles` 新欄位（`trial_started_at` / `trial_expires_at` / `trial_questions_used`）、RPC `increment_trial_question()`、header `.trial-banner`、paywall `trial_ended` / `trial_upgrade` 文案。

### 2.7 邀請好友雙方各拿 14 天（v14，2026-04-20 上線）

- **邀請資格**：只有正式付費用戶（standard / pro，且 subscription active）能發邀請；試用用戶**不算**付費，不能邀請也不算被推薦獎勵觸發對象。
- **獎勵觸發**：被推薦人首次付費成功即發放，無冷靜期；雙方 `current_period_end += 14 天`（延長現有訂閱）。
- **防濫用**：`referrals.referred_id UNIQUE`（只能被推一次）；`referrer_id <> referred_id` check；webhook 發獎前再檢一次推薦人仍為付費狀態，若失效則 `status='rejected'` 不發獎。
- **前端**：`#/referral` 頁（連結複製、統計、邀請紀錄）、user dropdown「💝 邀請好友拿 14 天」、URL `?ref=CODE` 捕獲 → localStorage → 登入後呼叫 `claim_referral` RPC 綁定。
- **隱私**：推薦紀錄列表顯示被推薦人「首字+**」（SQL 層遮罩），不洩漏 email / 全名。
- **相關檔**：`supabase_schema_v14_referral.sql`、新表 `referrals`、RPC `claim_referral` / `apply_referral_reward` / `get_my_referral_stats`、`payment/edge_webhook.ts` 在 subscription 建立後呼叫 `apply_referral_reward`。
...
## 4. 7 大考試系統分類（2026-04-17 白話化命名）

| ID | 名稱（白話） | 舊名稱 | Icon | 考試數 | desc 舉例 |
|---|---|---|---|---|---|
| tech-cert | 技術士證照 | 國家技術士技能檢定 | 🔧 | 61 | 甲級・乙級・丙級｜室內配線、中餐烹調、電腦應用等 |
| prof-exam | 專技人員考試 | 專門職業及技術人員考試 | 🏛 | 28 | 醫師・律師・會計師・建築師・護理師・導遊領隊等 |
| civil | 公職考試 | 公務人員考試 | 📋 | 24 | 高考・普考・初考・地方特考・警察特考等 |
| language | 語言檢定 | 語言能力檢定 | 🌐 | 17 | 多益・全民英檢・雅思・托福・JLPT 日語等 |
| academic | 升學考試 | （不變） | 🎓 | 8 | 學測・分科測驗・統測・國中教育會考 |
| finance | 金融證照 | 金融商業證照 | 💼 | 28 | 信託・證券・期貨・保險業務員、CFP/AFP 等 |
| it-cert | IT 國際認證 | 資訊科技國際認證 | 💻 | 25 | AWS・Azure・GCP、CompTIA、CCNA、PMP 等 |

**命名原則**：去掉政府術語，用白話讓用戶一眼辨識。desc 改為「舉例式」，列出代表性考試讓用戶不用猜。

目前 `exams` 陣列有 **191 筆考試 metadata**，其中 **40 個考試有完整官方題庫**（甲種職安衛 153 題 + 39 個新北勞動雲批次 6,931 題 + 308 張圖片題 = 共 7,393 題）。其餘顯示「📚 資料完整，題庫建置中」。

**重要 category ID 命名**（不要再踩雷）：
- `tech-cert`、`prof-exam`、`civil`、`language`、**`academic`**（不是 entrance）、`finance`、`it-cert`
- 前端顯示名稱已從官方用語改為白話（如「國家技術士技能檢定」→「技術士證照」），但 ID 不變
...
- ✅ 首頁、考試目錄、個人中心、定價頁
- ✅ 7 大系統分類架構
- ✅ **191 個考試 metadata 全線完成**（技術士 61 + 專技 28 + 金融 28 + IT 25 + 公職 24 + 語言 17 + 升學 8）
...
- ✅ **首頁優化 + 定價重構 + 分類白話化**（2026-04-17）：
  - **Header 縮小**：`--header-h` 從 80px → 60px，header padding/logo/nav 同步微調
  - **Hero 區縮小**：padding 72→48px、hero-sub 字體 18→17px、間距收窄
  - **首頁文案修正**：「不用帳號」→「免費註冊即可開始」；「章節練習」→「主題練習」；免費 CTA 改跳 `openLoginModal()`
  - **三層定價重構**：從舊版二層（標準 NT$199 / 進階 NT$349）→ 免費版 + 標準方案 NT$99 / 30 天 + 完整方案 NT$249 / 30 天
  - **標準方案設計**：限定技術士證照 + 金融證照；含 AI 弱點分析和學習進度，不含考前衝刺包（作為升級誘餌）
  - **Pro 折扣視覺強化**：`.discount-tag` 紅色標籤 + `pulse-discount` 動畫
  - **分類名稱白話化**：7 大分類從官方用語改為口語（如「國家技術士技能檢定」→「技術士證照」）
  - **分類 desc 改舉例式**：列出代表性考試名稱，讓用戶一眼辨識
  - **Bug fix**：`q.exp` 為 null 時解析顯示「null」文字，改為條件渲染
  - **模擬考題數**：定價卡片移除「（80 題）」硬編碼，因不同科目題數不同
...
### 功能開發（已討論可行性）
- [ ] **學習進度視覺化**：已有 70% 基礎設施（exam_progress, chapter_progress 表），需 2-3 天做趨勢圖表
- [ ] **考前衝刺包**：Route A — 加 `exam_frequency` 欄位標註高頻題（1-2 天）；Route B — AI 個人化推薦（3-5 天）。建議先做 Route A
