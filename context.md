...
| 單科方案 | NT$99/月 | NT$249/季（省 16%） | 價格敏感用戶，只考一科 |
| 完整方案 | NT$249/月 | NT$599/季（省 20%） | 高付費意願用戶，全功能 |

### 2.2 功能分層

| 功能 | 免費版 | 單科方案 | 完整方案 |
|---|---|---|---|
| 題庫練習 | 所有考試每日 10 題 | 任選 1 科無限練習 | 所有考試無限練習 |
| 主題練習與隨機測驗 | 基本 | ✅ | ✅ 無限制 |
| 模擬考 | ❌ | ✅ 該科完整模擬考 | ✅ 所有考試 |
| 答題解析 | ✅ | ✅ | ✅ + AI 補充說明 |
| AI 弱點分析 | ❌ | ✅ | ✅ + 個人化學習計畫 |
...
不開放單科的類別（付費意願高，不需降價）：專技人員考試、公職考試、語言檢定、升學考試、IT 國際認證

### 2.4 定價策略決策紀錄

- **競品參考**：阿摩 VIP 約 NT$130-140/月，有 iOS+Android app，內容為 UGC 模式品質參差
- **定價高於阿摩的理由**：AI 功能、結構化題庫、模擬考、弱點分析，品質定位更高
- **設定較高定價的考量**：日常價格留出折扣空間，後續可做「首月 NT$69」「限時七折」等促銷活動
- **單科方案存在理由**：降低技職/金融用戶門檻，避免托福/高普考用戶降級（限定適用科目）
- **考前衝刺包不放入單科的理由**：最佳升級觸發點——考前 2-3 週焦慮感上來時，用戶最容易轉換
- **升級路徑設計**：單科用戶升級完整方案時，按比例折抵剩餘天數
...
- ✅ **首頁優化 + 定價重構 + 分類白話化**（2026-04-17）：
  ...
  - **三層定價重構**：從舊版二層（標準 NT$199 / 進階 NT$349）→ 免費版 + 單科 NT$99/月 + 完整 NT$249/月
  - **單科方案設計**：限定技術士證照 + 金融證照；含 AI 弱點分析和學習進度，不含考前衝刺包（作為升級誘餌）
