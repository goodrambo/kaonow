...
-- ═══════════════════════════════════════════════════════════════════
-- 0. 預備：補齊所有可變內容表的 updated_at 欄位（一致性稽核）
-- ═══════════════════════════════════════════════════════════════════

-- 4 個缺少 updated_at 的表：exam_categories / chapters / exam_news / promo_codes
ALTER TABLE public.exam_categories ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();
ALTER TABLE public.chapters        ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();
ALTER TABLE public.exam_news       ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();
ALTER TABLE public.promo_codes     ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();

-- 加 trigger 自動更新 updated_at（使用既有 set_updated_at 函數）
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_exam_categories_updated') THEN
    CREATE TRIGGER trg_exam_categories_updated BEFORE UPDATE ON public.exam_categories
      FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_chapters_updated') THEN
    CREATE TRIGGER trg_chapters_updated BEFORE UPDATE ON public.chapters
      FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_exam_news_updated') THEN
    CREATE TRIGGER trg_exam_news_updated BEFORE UPDATE ON public.exam_news
      FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_promo_codes_updated') THEN
...
ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, chapter_number = EXCLUDED.chapter_number, sort_order = EXCLUDED.sort_order, updated_at = NOW();
